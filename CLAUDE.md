...
- `/profile/[section]/[profile]` — Full-bleed image page with a description overlay
...
**The profile route is the exception**: it reads a static JSON catalogue (`lib/data/profiles.json`) through `lib/profiles.ts` — no Sanity, no backend, no fetch. See `lib/data/README.md` for the schema. Server component resolves params → `getProfileView()` narrows the payload → the client component owns all interaction.

### The profile page (`/components/Profile/`)
A fixed, non-scrolling white surface that opts out of the site's dark shell. `Navigation` hides itself on `/profile` (and `/studio`) — it would land on top of the page's own header.
...
- Mobile uses `100dvh` and `env(safe-area-inset-*)`; the breakpoint (750px) lives in `components/Profile/_grid.scss` alongside the 12-column layout mixins ported from the source site.
